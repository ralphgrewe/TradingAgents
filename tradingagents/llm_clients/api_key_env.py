...
PROVIDER_API_KEY_ENV: dict[str, str | None] = {
    ...
    "ollama":     None,
    # Intel XPU: local in-process model, no server/auth.
    "intel_xpu":  None,
    ...
    # Fork-only: Perplexity Agent API / chat completions (see
    # perplexity_client.PerplexityClient). Not part of the OpenAI-compatible
    # registry (different wire format) but still registered here so the CLI
    # key-prompt flow and any other PROVIDER_API_KEY_ENV consumer can find it.
